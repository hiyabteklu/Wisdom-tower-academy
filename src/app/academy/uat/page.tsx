  ...
  return (
    <div className="relative min-h-[80vh]">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-1/4 w-[28rem] h-[28rem] rounded-full blur-3xl opacity-30 bg-gradient-to-br from-emerald-500/20 via-teal-500/5 to-transparent" />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        ...
        <div className="mb-8 animate-fade-up">
          ...
          <p className="text-wisdom-muted text-lg max-w-2xl leading-relaxed">
            University Admission Test preparation with practice, mocks, and strategies.
          </p>
        </div>
        ...
        <div className="perspective-scene grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 stagger-children">
          {resourceHubs.map((hub) => (
            <Link
              key={hub.id}
              href={`/academy/uat/${hub.id}`}
              className={`card-3d group relative overflow-hidden rounded-3xl border border-white/12 bg-wisdom-card p-6 md:p-7 hover:border-emerald-400/35 shadow-lg ${hub.glow}`}
            >
              <div
