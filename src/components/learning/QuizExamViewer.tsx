    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-1.5">
        {questions.map((qq, i) => {
          ...
          let cls =
            "border-white/15 text-wisdom-muted bg-transparent"; // skipped / unanswered
          if (i === idx) {
      ...
      {submitted && (
        <div className="rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-5 space-y-4">
          ...
          {/* Exam review with All / Missed filters */}
          {isExam && (
                  ...
                  >
                    ...
                    <div className="space-y-1 text-xs">
                      <p className={correct ? "text-emerald-300" : "text-rose-300"}>
                        Your answer:{" "}
                        {ans != null
                          ? `${String.fromCharCode(65 + ans)}. ${qq.choices?.[ans] ?? ans}`
                          : "Skipped"}
                      </p>
                      {missed && (
                        <p className="text-emerald-200">
                          Correct:{" "}
                          {qq.correct != null
                            ? `${String.fromCharCode(65 + qq.correct)}. ${qq.choices?.[qq.correct] ?? qq.correct}`
                            : "—"}
                        </p>
                      )}
                      {qq.solution && (
