  ...
  return (
    <div className="relative min-h-[80vh]">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-1/4 w-[28rem] h-[28rem] rounded-full blur-3xl opacity-30 bg-gradient-to-br from-rose-500/20 via-red-500/5 to-transparent" />
      </div>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        ...
        <div className="mb-8 animate-fade-up">
          ...
          <h1 className="font-display text-4xl md:text-5xl font-extrabold tracking-tight mb-3">
            <span className="text-rose-400">GAT</span> resources
          </h1>
          <p className="text-wisdom-muted text-lg max-w-2xl leading-relaxed">
            Graduate Admission Test coaching — quantitative, verbal, and analytical.
          </p>
        </div>
        ...
        <div className="max-w-3xl mb-12">
          <AcademicResultSaver scopeId="gat" scopeLabel="GAT" accent="text-rose-400" />
        </div>
        ...
        <div className="perspective-scene grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 stagger-children">
          {resourceHubs.map((hub) => (
            <Link
              key={hub.id}
              href={`/academy/gat/${hub.id}`}
              className={`card-3d group relative overflow-hidden rounded-3xl border border-white/12 bg-wisdom-card p-6 md:p-7 hover:border-rose-400/35 shadow-lg ${hub.glow}`}
            >
              <div
