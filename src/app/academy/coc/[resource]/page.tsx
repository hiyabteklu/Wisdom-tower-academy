...
export default async function CocResourcePage({
  params,
}: {
  ...
  return (
    <div className="relative min-h-[75vh]">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-1/4 w-80 h-80 rounded-full blur-3xl opacity-25 bg-gradient-to-br from-indigo-500/20 via-blue-500/5 to-transparent" />
      </div>

      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        <CategoryBackButton fallback="/academy/coc" />
        ...
        <div className="mb-8">
          <AcademicResultSaver
            scopeId={`coc-${resource.id}`}
            scopeLabel={`COC · ${resource.name}`}
        ...
        <div className="rounded-3xl border border-white/15 bg-wisdom-card shadow-card-3d overflow-hidden animate-fade-up">
          <div className="px-6 sm:px-8 py-10 text-center">
            ...
            <p className="text-wisdom-muted text-sm max-w-md mx-auto leading-relaxed mb-8">
              COC {resource.name.toLowerCase()} materials will appear here. Track scores above.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link
                href="/academy/coc"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/15 text-sm font-medium hover:border-indigo-400/40 hover:text-indigo-400 transition-colors"
              >
                All COC hubs
              </Link>
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-500 text-white text-sm font-semibold hover:bg-indigo-400 transition-colors"
              >
